...
export function EvaluationExplorer({ report }: { report: EvaluationReport }) {
  ...
  return (
    <>
      <section className="metricGrid" aria-label="Evaluation metrics">
        {report.metrics.map((metric, index) => {
          ...
          return (
            <article className={`metricCard ${index === 0 ? "metricPrimary" : ""}`} key={metric.id}>
              ...
              <div className="metricValues"><div><small>Baseline</small><strong>{metric.baseline}{metric.unit}</strong></div><span /><div><small>KisanPath</small><strong>{metric.final}{metric.unit}</strong></div></div>
      ...
      <section className="safetyStrip">
        ...
        <div><strong>Zero false-eligibility outcomes in the frozen seed set</strong><p>Deterministic failures remained blocking and missing facts triggered clarification.</p></div>
      ...
      <section className="caseExplorer" aria-labelledby="case-explorer-title">
        <header className="caseExplorerHeader">
          <div><span className="eyebrow"><FlaskConical size={13} />Frozen synthetic dataset</span><h2 id="case-explorer-title">Case explorer</h2></div>
          <div className="filterPills" aria-label="Filter cases by language">
            {(["all", "gu", "hi", "en"] as const).map((value) => <button type="button" className={language === value ? "active" : ""} aria-pressed={language === value} onClick={() => setLanguage(value)} key={value}>{value === "all" ? "All" : value.toUpperCase()}</button>)}
          </div>
        </header>
        <div className="caseExplorerBody">
          <div className="caseList" role="list" aria-label="Evaluation cases">
            {cases.map((item) => (
          ...
          {selected && (
            <article className="caseDetail">
              <span className="eyebrow">Sanitized input</span>
              <blockquote lang={selected.language}>“{selected.input}”</blockquote>
              <div className="caseOutcomes"><div><small>Expected</small><EligibilityStatusBadge status={selected.expected} /></div><div><small>Actual</small><EligibilityStatusBadge status={selected.actual} /></div></div>
              <div className="auditNote"><strong>Audit note</strong><p>{selected.note}</p></div>
              <div className="traceLine"><span /><span /><span /><span /><span /></div>
              <p className="traceCaption">parse input → update profile → evaluate rules → verify evidence → compose response</p>
            </article>
          )}
        </div>
      </section>
